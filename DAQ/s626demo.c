//******************************* 626demo.c **************************************
//* This is a demo/sample application program that shows you how to use the 626
//* loadable Linux device driver 's626drv' and the API functions of 626 core&mod 
//* driver library 'libs626.a' to develop your own application(s) for any projects
//* using Sensoray Model 626 under Linux.
//*
//* Copyright (c) 2004.  Sensoray Co., Inc
...
WORD				cntr_chan = CNTR_2B;	// Tested, using all counters to generate interrupts:
...
int main( int argc, char *argv[] )
{
  ...
  printf("Board(%i) -  ErrFlags = 0x%lx \n", board, errFlags );

  if (errFlags!=0x0) 
  {
	  printf("Board open/installation with Err = 0x%lx : ", errFlags);
	  switch (errFlags)
	  {
		  case ERR_OPEN:
  ...
  printf ("Found %d 626 board%c\n", numboards, numboards > 1 ? 's' : ' ');
  for (i = 0; i < numboards; i++) printf ("Board(%d) is on PCI bus %d; slot %d\n\n",
											i, S626_GetAddress (i) >> 16, S626_GetAddress (i) & 0xffff);
  // print Main Menu.
  MainMenu();
  ...
  for (;;)
  {
    ...
    if ( fgets (cmdbuff, MAXBUF, stdin) != NULL )
    {
      ...
      if ( cmdbuff[0] != 0 )
      {
		  ...
		  // for command = '1', '2', ..., numboards.
		  if ( ch>='1' && ch<='9' && (ch-'0')<=numboards )
		  {
			  Toggle (ch-'0'-1);	//toggle the corresponding boards watchdog state.
			  continue;
		  }
  ...
  // close all boards.
  for (i = 0; i < numboards; i++) S626_CloseBoard (i);	//close 626 boards
...
// Poll List (EOPL) bit set.
  ...
  for (;;)
  {
    ...
    printf ("\ttime_spent = %lu (ms) to get %i samples -- Detected sampling rate = %f kHz \n", time_spent, samples, samplingRate);
...
void TestDO( DWORD board )
{
  ...
  printf ("Enter DO Group # and toggling times [0-2 1-%li, q] : ", max);

  for (;;)
  {
	  fgets (cmdbuff, MAXBUF, stdin);
	  if (cmdbuff[0] == 'q') return;
	  sscanf( cmdbuff, "%i %ld", &bank, &times );
	  ...
	  printf ("\ttime_spent = %lu (ms) to toggle %li times -- Detected switching rate = %f kHz \n", time_spent, times, switchingRate );
...
void MeasureDI( DWORD board )
{
  ...
  for (;;)
  {
	  ...
	  printf ("\ttime_spent = %ld (ms) for read-in %li times -- Detected read-in rate = %f kHz \n", time_spent, times, readinRate );
...
void TestDAC( DWORD board )
{
  ...
  long int			cnt, times;
  // DA: chan size changed from WORD to unsigned long.  stack smashing error in sscanf otherwise
  unsigned long		        chan;
  int				i, outLast = 0;
//  long int			loop;
  ...
  for (;;)
  {
	  ...
	  if ( chan>3 ) 
	  {
		  printf ("%lu Enter DAC channel # and ramping outputs [0-3 1-%li, q] : ",chan, max );
		  continue;
	  }
	  if ( times<1 || times>max ) 
	  {
	  ...
	  printf ("DAC channel #%lu ramping (-10V_/+10V) ..... ", chan );	fflush(stdout);
	  ...
	  printf ("\ttime_spent = %lu (ms) for %li analog outputs -- Detected analog-out rate = %f kHz \n", time_spent, times, aoutRate );

	  // last DAC output
	  printf ("\tChannel #%lu :  DAC output [%ld] = %6i ( %f V ) \n", chan, cnt, outLast, 10.0*(float)(outLast)/8191.0);
...
void EncoderTest( DWORD board )
{
	int            Counter = CNTR_0A;	// CNTR_0A:	inputs on J5: CNTR_0A - A-,A+,B-,B+,I-,I+
	...
	for ( ; ; )
	{
		...
		if ( cmd[0]=='F' || cmd[0]=='f' ) 
		{
			...
			printf( "\tForwarding ..... " );	fflush(stdout);
		...
		} else if ( cmd[0]=='R' || cmd[0]=='r' || cmd[0]=='B' || cmd[0]=='b') 
		{
			time(&time_now);
			printf ("\tStarted at %s", ctime(&time_now));
			printf( "\tReversing ..... " );	fflush(stdout);
