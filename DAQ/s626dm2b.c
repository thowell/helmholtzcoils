...
*/
  ...
  if (errFlags!=0x0) 
  {
	  printf("Board #0 open/installation with Err = 0x%lx : ", errFlags);
	  switch (errFlags)
	  {
		  case ERR_OPEN:
  ...
  if (errFlags!=0x0) 
  {
	  printf("Board #1 open/installation with Err = 0x%lx : ", errFlags);
	  switch (errFlags)
	  {
		  case ERR_OPEN:
			  printf("\t\t Can't open driver.\n"); break;
...
void TestDO( DWORD board )
{
  ...
  for (;;)
  {
	  fgets (cmdbuff, MAXBUF, stdin);
	  if (cmdbuff[0] == 'q') return;
	  sscanf( cmdbuff, "%d %ld", &bank, &times );
	  ...
	  printf ("\ttime_spent = %ld (ms) to toggle %li times -- Detected switching rate = %f kHz \n", time_spent, times, switchingRate );
...
void TestDAC( DWORD board )
{
  ...
  for (;;)
  {
	  ...
	  printf ("DAC channel #%li ramping (-10V_/+10V) ..... ", chan );	
	  fflush(stdout);
